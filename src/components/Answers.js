...
const Answers = (props) => {
	...
	return (
		<div
			className={`mx-2 h-24 rounded-md hover:drop-shadow-md ${isSelected ? 'isSelectedStyle' : 'isNotSelectedStyle'}`}
			onClick={answerHandler}
			style={answerBox}
>
			<h1 className={`text-center text-black w-full h-full ${!isSelected ? 'notSelectedAnswerText' : 'selectedAnswerText'}`}>
				{props.text}
			</h1>
		</div>
	)
